namespace L06_SpaceInvaders {
    ...
    export let rootEnemyProjectiles: fc.Node = new fc.Node("Enemy Projectiles");
    rootEnemyProjectiles.addComponent(new fc.ComponentTransform());
    root.addChild(rootEnemyProjectiles);

    let enemyShoots: boolean = false;
    let shootingEnemy: Enemy;
    let shootingEnemyBullet: Projectile;
    ...
    export let peng: HTMLAudioElement;

    let explosion: HTMLAudioElement;

    let buttonMusic: HTMLButtonElement;

    export let audioAllowed: boolean = false;
    ...
    function update(_event: Event): void {
        ...
        //Enemy bullets
        for (let enemyProjectile of rootEnemyProjectiles.getChildren() as Projectile[]) {
            enemyProjectile.flyEnemy();
            if (enemyProjectile.mtxLocal.translation.y < -1.5)
                rootEnemyProjectiles.removeChild(enemyProjectile);
        }
        ...
        if (enemyShoots) {
            enemyShoots = false;
            shootingEnemyBullet = <Projectile>shootingEnemy.getChild(0).getChild(shootingEnemy.getChildren().length - 1);
    ...
    function shouldEnemyShootAndWhichOne(): void {
        let random: number = Math.random();
        
        if (random >= 0.98) {
            enemyShoots = true;

            let activeEnemys: Enemy[] = <Enemy[]>invadersSmall.getChildren();
            let randomEnemy: number = Math.random() * activeEnemys.length;
    ...
    function hndlMusic(): void {
        audioAllowed = !audioAllowed;
        if (audioAllowed) {
            buttonMusic.innerHTML = "Music Button (Music on)";
            backgroundAudio = <HTMLAudioElement>document.getElementById("BackgroundMusic");
            if (backgroundAudio) {
